    ...
    function addRank(issue, index) {
        issue.querySelectorAll("section.ghx-stat-fields, div.ghx-plan-main-fields").forEach(function(fieldContainer) {
            if (!fieldContainer.querySelector("div.ghx-stat-0") && !issue.classList.contains("ghx-done")) {
                fieldContainer.prepend(utils.createElementFromHTML(issue, '<div class="ghx-row ghx-stat-0" style="margin-right: 3px; padding-top: 2px; display: inline-block; text-decoration: overline"><span data-tooltip="Visual Rank Only" original-title="" title="Visual Rank Only"><b>' + (index + 1) + '</b></span></div>'));
            }
        });
    }
            ...
            utils.pagedJIRA(baseUrl + "/rest/api/2/search?jql=issuetype in subTaskIssueTypes() and parent=" + key + "&fields=key,status,statuscategorychangedate,assignee,summary,issuetype&", "issues").then(function(subtasks) {
                ...
                subtasks.forEach(function(subtask) {
                    ...
                    subtaskDataTable += ' ' +
                        ...
                        '                <span role="img" aria-label="' + (subtask.fields.assignee ? subtask.fields.assignee.displayName : '') + '" ' +
                        '                    style = "background-color: transparent; background-image: url(&quot;' + (subtask.fields.assignee ? subtask.fields.assignee.avatarUrls['24x24'] : '') + '&quot;); ' +
                        '                    background - position: center center; background - repeat: no - repeat; background - size: cover; border - radius: 50 %; display: flex; flex: 1 1 100 %; height: 100 %; width: 100 %; " >' +
                        '                </span > ' +
                        '            </div> ' +
                        '        </div> ' +
